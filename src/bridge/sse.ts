...
export function encodeResponseSse(built: BuiltCodexResponse): string {
  ...
  const events: string[] = [
    ...
    event("response.output_item.added", { output_index: 0, item: pendingItem(item) }),
  ];
  if (item.type === "message") {
    ...
    events.push(
      ...
      event("response.output_text.done", { item_id: itemId, output_index: 0, content_index: 0, text }),
      event("response.content_part.done", { item_id: itemId, output_index: 0, content_index: 0, part: content[0] ?? {} }),
    );
